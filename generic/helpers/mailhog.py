...
class MailhogApi:
    ...
    def get_api_v2_messages(self, limit: int = 50) -> Response:
        with allure.step("Получение писем по определенному лимиту"):
            """
    ...
    def get_token_from_last_email(self) -> str:  # если тест выше выполнился , то из него мы будем брать токен
        with allure.step("Получение авторизационного токена из последнего полученного письма"):
            """
    ...
    def get_token_by_login(self, login: str, attempt=50):  # для получения письма от определенного пользователя
        with allure.step("Получение авторизационного токена от определенного пользователя"):
            if attempt == 0:
    ...
    def delete_all_messages(self):
        with allure.step("Удаление всех полученных писем"):
